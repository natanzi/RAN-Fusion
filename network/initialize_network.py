...
def initialize_network(gNodeBs_config, cells_config, sectors_config, ues_config, db_manager):
    ...
    # Link Sectors to Cells
    # Assuming each cell has a method to add sectors
    for cell in cells.values():
        for sector in sectors:
            if sector.cell_id == cell.id:
                cell.add_sector(sector)
    
    # Link Cells to gNodeBs
    # Assuming each gNodeB has a method to add cells
    for gnb in gNodeBs.values():
        for cell in cells.values():
            if cell.gNodeB_id == gnb.id:
                gnb.add_cell(cell)
